        ...
        if (StringUtils.hasText(token)) {
            try {
                ...
                if (!jwtTokenProvider.validateToken(token)) {
                    ...
                    // 2. Check if token was blacklisted (logout scenario)
                    log.debug("Blacklisted JWT token used for: {}", request.getRequestURI());
                } else if (!jwtTokenProvider.isAccessToken(token)) {
                    // 3. Reject refresh tokens used as access tokens
                    log.debug("Refresh token used as access token");
                } else {
